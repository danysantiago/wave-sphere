...
void SoftSerial_init(void) {
	...
	TA0CCTL2 = SCS | CM1 | CAP | CCIE;   // Sync TA1CLK and MCLK, Detect Neg Edge, Enable Capture mode and RX Interrupt
...
void SoftSerial_xmit(uint8_t byte) {
	...
	TA0CCR1 = TA0R;               // resync with current TimerA clock
	TA0CCR1 += TICKS_PER_BIT;    // setup the next timer tick
	TA0CCTL1 = OUTMOD0 + CCIE;   // set TX_PIN HIGH and reenable interrupts
 ...
 * NOTE: The comments below are for transmitting data.
 * Handle the sending of a data byte with one
 * start bit + 8 data bits + stop bit.
 *
 * NOTE: The comments below are for receiving data.
...
__interrupt void SoftSerial_RX_ISR(void) {
	...
	case  2: // CCR1
		if (txBitCnt == 0) {                // All bits TXed?
			TA0CCTL1 &= ~CCIE;               // disable interrupt, used as a flag to SoftSerial_xmit
			txBitCnt = 10;                  // Re-load bit counter
		} else {
			TA0CCR1 += TICKS_PER_BIT;        // setup next time to send a bit

			if (USARTTXBUF & 0x01) {        // look at LSB and decide what to send
				TA0CCTL1 &= ~OUTMOD2;        // TX Mark '1'
			} else {
				TA0CCTL1 |= OUTMOD2;         // TX Space '0'
			}
		...
		break;
	case  4: // CCR2
		TA0CCR2 += TICKS_PER_BIT;             // Setup next time to sample
		if (TA0CCTL2 & CAP) {                 // Is this the start bit?
			TA0CCTL2 &= ~CAP;                 // Switch capture to compare mode
			TA0CCR2 += TICKS_PER_BIT_DIV2;    // Sample from the middle of D0
		}
		else {
			rxData >>= 1;
			if (TA0CCTL2 & SCCI) {            // Get bit waiting in receive latch
				rxData |= 0x80;
			}
			rxBitCnt--;
			if (rxBitCnt == 0) {             // All bits RXed?
				...
				TA0CCTL2 |= CAP;              // Switch compare to capture mode
				TA0CCR2 += TICKS_PER_BIT;     // account for the stop bit
			}
		}
		break;
