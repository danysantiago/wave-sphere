		...
		unsigned char *data)
{
	...
	__data16_write_addr((unsigned short) &DMA0DA, (unsigned long) data);
...
sends the next byte. */
	...
	__data16_write_addr((unsigned short) &DMA1SA, (unsigned long)&blank);
		...
		unsigned char *data)
{
	...
	__data16_write_addr((unsigned short) &DMA0DA,(unsigned long)&UCB3TXBUF);
	/* The size of the block to be transferred */
	DMA0SZ = SD_BLOCKSIZE;
	/* Configure the DMA transfer*/
	DMA0CTL =
	 ...
	 * token */
	UCB3TXBUF = SD_TOK_WRITE_STARTBLOCK;
